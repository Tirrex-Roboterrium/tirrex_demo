...
def get_edge_detection_configuration_file_path(context):
    return os.path.join(get_robot_config_directory(context), 'edge_detection_circle.yaml')
...
def launch_setup(context, *args, **kwargs):
    ...
    edge_detection = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            get_package_share_directory("romea_edge_detection_circle")
            + "/launch/edge_detection_circle.launch.py"
        ),
        launch_arguments={
            "mode": mode,
